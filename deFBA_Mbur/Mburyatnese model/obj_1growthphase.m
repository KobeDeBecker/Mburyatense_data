...
% a non-linear least squares problem in case only one (exponetial) growth
% phase is present. 
...
function obj = obj_1growthphase(p,X1,X2,X3)
...
% Simulate model data
lnX1_m = model_1growthphase(p1,X1(:,1));
lnX2_m = model_1growthphase(p2,X2(:,1));
lnX3_m = model_1growthphase(p3,X3(:,1));
